...
本模組為純標準函式庫實作，不依賴 pandas/numpy，以支援輕量級雲端環境 (Render) 運作。
"""
...
def _as_float(v: Any, default: Any = 0.0) -> Any:
    if v is None or v == "":
        return default
    try:
        return float(v)
...
def _load_per_rows(code: str, as_of: str) -> list[dict]:
    ...
    for r in rows:
        ...
        if dt and dt <= as_of:
            out.append({
                "date": dt,
                "PER": _as_float(r.get("PER"), None),
                "PBR": _as_float(r.get("PBR"), None),
                "dividend_yield": _as_float(r.get("dividend_yield"), 0.0)
            })
    out.sort(key=lambda x: x["date"])
    return out

def _load_chips_data(code: str, as_of: str) -> dict[str, dict]:
    rows = _read_csv(CACHE_DIR / f"{code}_chips.csv")
    by_date = {}
    for r in rows:
        dt = r.get("date", "")
        if not dt or dt > as_of:
            continue
        buy = _as_float(r.get("buy"), 0.0)
        sell = _as_float(r.get("sell"), 0.0)
        net = buy - sell
        name = r.get("name", "")
        
        if dt not in by_date:
            by_date[dt] = {"net": 0.0, "foreign_net": 0.0, "trust_net": 0.0}
...
def _load_margin_data(code: str, as_of: str) -> list[dict]:
    rows = _read_csv(CACHE_DIR / f"{code}_margin.csv")
    out = []
    for r in rows:
        dt = r.get("date", "")
        if dt and dt <= as_of:
            out.append({
                "date": dt,
                "MarginPurchaseTodayBalance": _as_float(r.get("MarginPurchaseTodayBalance"), 0.0)
            })
    out.sort(key=lambda x: x["date"])
    return out
...
def _load_revenue_yoy(code: str, as_of: str) -> tuple[float | None, list[float]]:
    ...
    parsed = []
    for r in rows:
        year = str(r.get("revenue_year") or "").strip()
        month = str(r.get("revenue_month") or "").strip()
        rev = _as_float(r.get("revenue"), 0.0)
        if not year or not month or rev <= 0:
            continue
        try:
    ...
    for i in range(len(parsed)):
        curr_p = parsed[i]
        curr_yr = int(curr_p["period"][:4])
        curr_mo = int(curr_p["period"][5:7])
        
        prev_yr = curr_yr - 1
        prev_p_str = f"{prev_yr:04d}-{curr_mo:02d}-01"
        
        prev_rev = None
        for j in range(i):
            if parsed[j]["period"] == prev_p_str:
                prev_rev = parsed[j]["revenue"]
                break
                
        if prev_rev is not None and prev_rev > 0:
            yoy = curr_p["revenue"] / prev_rev - 1.0
            if curr_p["announce"] <= as_of:
                yoy_by_announce.append({
                    "announce": curr_p["announce"],
                    "yoy": yoy
                })
                
    yoy_by_announce.sort(key=lambda x: x["announce"])
    latest_yoy = yoy_by_announce[-1]["yoy"] if yoy_by_announce else None
    recent_yoy_list = [x["yoy"] for x in yoy_by_announce[-3:]] if len(yoy_by_announce) >= 3 else [x["yoy"] for x in yoy_by_announce]
    return latest_yoy, recent_yoy_list
...
def calculate_potential_score(symbol: str, as_of: str) -> dict:
    ...
    try:
        ...
        start_dt = (datetime.fromisoformat(as_of) - timedelta(days=420)).date().isoformat()
        end_dt = as_of
        
        hist_rows = fetch_history(symbol, start_dt, end_dt)
        hist_rows = [r for r in hist_rows if r["date"] <= as_of]
        hist_rows.sort(key=lambda x: x["date"])
    except Exception as e:
        fallback_res["warnings"] = [f"無法載入該股價格歷史: {str(e)}"]
        return fallback_res

    if not hist_rows or len(hist_rows) < 30:
        fallback_res["warnings"] = ["歷史股價交易日數不足 30 天，無法評分"]
        return fallback_res
        
    try:
        closes = [_as_float(r.get("close")) for r in hist_rows]
        volumes = [_as_float(r.get("volume")) for r in hist_rows]
        highs = [_as_float(r.get("high")) for r in hist_rows]
        lows = [_as_float(r.get("low")) for r in hist_rows]
        dates = [r.get("date") for r in hist_rows]
        
        close = closes[-1]
        
        # 載入 PER/PBR 等輔助基本面
        per_rows = _load_per_rows(clean_sym, as_of)
        per_vals = [r["PER"] for r in per_rows if r["PER"] is not None]
        pbr_vals = [r["PBR"] for r in per_rows if r["PBR"] is not None]
        
        per = per_vals[-1] if per_vals else None
        pbr = pbr_vals[-1] if pbr_vals else None
        
        div_yield = per_rows[-1]["dividend_yield"] if per_rows else 0.0
        
        median_per = _median(per_vals[-250:], 15.0)
        median_pbr = _median(pbr_vals[-250:], 2.0)
        
        rev_yoy, rev_recent = _load_revenue_yoy(clean_sym, as_of)
        ...
        accel_pts = 0.0
        if rev_recent and len(rev_recent) >= 2:
            prev_avg_yoy = _mean(rev_recent[:-1])
            if (rev_yoy or 0.0) > prev_avg_yoy:
                accel_pts = 5.0
                    
        eps_pts = 0.0
        # 建立合併價格與 PER/PBR 的歷史數據
        per_by_date = {r["date"]: r for r in per_rows}
        merged_df = []
        for r in hist_rows:
            dt = r["date"]
            p_row = per_by_date.get(dt)
            if p_row and p_row["PER"] is not None and p_row["PBR"] is not None and p_row["PER"] > 0:
                merged_df.append({
                    "date": dt,
                    "close": _as_float(r.get("close")),
                    "PER": p_row["PER"],
                    "PBR": p_row["PBR"],
                    "EPS": _as_float(r.get("close")) / p_row["PER"],
                    "ROE": p_row["PBR"] / p_row["PER"]
                })
        
        if len(merged_df) >= 90:
            eps_recent = _mean([x["EPS"] for x in merged_df[-10:]])
            eps_prev = _mean([x["EPS"] for x in merged_df[-90:-60]])
            if eps_recent > eps_prev:
                eps_pts += 5.0
        ...
        stab_pts = 2.0
        if len(merged_df) >= 60:
            roe_std = _std([x["ROE"] for x in merged_df[-60:]])
            if roe_std < 0.02:
                stab_pts = 4.0
                
        # Margin stability and balance change
        margin_rows = _load_margin_data(clean_sym, as_of)
        avg_vol = _mean(volumes[-60:]) if len(volumes) >= 60 else 1.0
        if len(margin_rows) < 6 or avg_vol == 0:
            margin_chg = 0.0
        else:
            cur_bal = margin_rows[-1]["MarginPurchaseTodayBalance"]
            prev_bal = margin_rows[-6]["MarginPurchaseTodayBalance"]
            margin_chg = (cur_bal - prev_bal) / (avg_vol / 1000.0)
        ...
        # Load chips for net buy sum
        chips_data = _load_chips_data(clean_sym, as_of)
        inst_net_20 = sum(chips_data.get(d, {}).get("net", 0.0) for d in dates[-20:])
        cat_inst = 0.0
        ...
        foreign_today = chips_data.get(dates[-1], {}).get("foreign_net", 0.0) if dates else 0.0
        trust_today = chips_data.get(dates[-1], {}).get("trust_net", 0.0) if dates else 0.0
        ...
        vol = _std(returns) if returns else 0.03
        vol_pts = 0.0
        if vol < 0.025:
            vol_pts = 5.0
        elif vol < 0.045:
        ...
        # ROE 下滑警訊
        if len(merged_df) >= 90:
            roe_recent = _mean([x["ROE"] for x in merged_df[-10:]])
            roe_prev = _mean([x["ROE"] for x in merged_df[-90:-60]])
            if roe_recent < roe_prev - 0.03:
                deductions += 10.0
        ...
        fallback_res["warnings"] = [f"計算潛力得分時發生錯誤: {str(e)}", tb_str]
        return fallback_res
